...
<!-- Horizontal Menu Bar (Platforms Sub-navigation) -->
<div class="menu_wrap">
    <div class="container">
        <nav class="menu_nav">
            <?php
            // Fetch platform terms dynamically
            $platforms = get_terms( array(
                'taxonomy'   => 'platform',
                'hide_empty' => false,
            ) );

            // Helper to match Platform Term slug with custom beautiful SVG icon
            if ( ! empty( $platforms ) && ! is_wp_error( $platforms ) ) :
                foreach ( $platforms as $platform ) :
                    ...
                    // 5. Default SVG Book/Video Course for others
                    else {
                    ...
                    // Active State class check
                    $active_class = ( is_tax('platform', $platform->term_id) ) ? 'active' : '';
                    ?>
                    <a class="menu_link <?php echo esc_attr( $active_class ); ?>" href="<?php echo esc_url( $url ); ?>">
                        ...
                        <?php 
                        // If it's android-games or pc-games add crown private badge just like FileCR
