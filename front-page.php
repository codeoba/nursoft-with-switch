...
<main class="container main_content">
    <div class="software_layout">
        <div class="software_main_col">
            <?php
            // 1. Latest Updates Block (Regardless of Platform)
            ...
            $latest_updates_query = new WP_Query( $latest_updates_args );

            if ( $latest_updates_query->have_posts() ) :
            ?>
            <section class="products_section">
                <!-- Section Header with Title -->
                <div class="section-header">
                    <h4><?php _e('Latest Updated Apps', 'nursoft'); ?></h4>
                    <a class="view-all-btn" href="<?php echo esc_url( home_url('/software/') ); ?>">
                        <?php _e('View All', 'nursoft'); ?>
                    </a>
                </div>
                ...
                <div class="product-grid">
                    <?php
                    while ( $latest_updates_query->have_posts() ) : $latest_updates_query->the_post();
                        ...
                        // Get Platform
                        $plats = wp_get_post_terms( get_the_ID(), 'platform' );
                        $platform_slug = ! empty( $plats ) && ! is_wp_error( $plats ) ? $plats[0]->slug : 'ms-windows';
                        $platform_name = ! empty( $plats ) && ! is_wp_error( $plats ) ? $plats[0]->name : 'Windows';
                        ...
                        <div class="card_wrap">
                            ...
                            <div class="card_data">
                                ...
                                <a class="card_primary card_<?php echo esc_attr( $platform_slug ); ?>" href="<?php echo esc_url( get_term_link( $plats[0] ) ); ?>">
                                    <?php
            ...
            $platforms = get_terms( array(
                'taxonomy'   => 'platform',
                'hide_empty' => true, // Only show platforms that have software posts
            ) );

            // Dynamically prioritize user's current platform block (second block after Latest Updates)
            $prioritized_slug = nursoft_get_prioritized_platform();
            if ( ! empty( $platforms ) && ! is_wp_error( $platforms ) ) {
                $prioritized_platform = null;
                $other_platforms = array();
                
                foreach ( $platforms as $p ) {
                    // Check for exact slug match or equivalent variations for the prioritized platform
                    $is_match = false;
                    if ( $p->slug === $prioritized_slug ) {
                        $is_match = true;
                    } elseif ( $prioritized_slug === 'ms-windows' && ( $p->slug === 'windows' || $p->slug === 'pc' ) ) {
                        $is_match = true;
                    } elseif ( $prioritized_slug === 'mac' && ( $p->slug === 'macos' || $p->slug === 'mac-os' || $p->slug === 'apple' ) ) {
                        $is_match = true;
                    } elseif ( $prioritized_slug === 'android' && ( $p->slug === 'android-os' || $p->slug === 'mobile' ) ) {
                        $is_match = true;
                    }
                    
                    // We only prioritize the first match to avoid dropping other terms
                    if ( $is_match && $prioritized_platform === null ) {
                        $prioritized_platform = $p;
                    } else {
                        $other_platforms[] = $p;
                    }
                }
                
                if ( $prioritized_platform ) {
                    array_unshift( $other_platforms, $prioritized_platform );
            ...
            if ( ! empty( $platforms ) && ! is_wp_error( $platforms ) ) :
                foreach ( $platforms as $platform ) :
                    ...
                    // Query 4 software items for this specific platform, sorted by latest modified!
                    $args = array(
                        'post_type'      => 'software',
                        'posts_per_page' => 4,
                        'orderby'        => 'modified',
            ...
            <section class="products_section">
                ...
                <div class="section-header">
                    ...
                    <a class="view-all-btn" href="<?php echo esc_url( get_term_link( $platform ) ); ?>">
                        <?php _e('View All', 'nursoft'); ?>
                    </a>
                </div>
                ...
                <div class="product-grid">
                    ...
                    while ( $query->have_posts() ) : $query->the_post();
                        ...
                        <div class="card_wrap">
                            ...
                            <div class="card_data">
                                <!-- Platform indicator link -->
                                <a class="card_primary card_<?php echo esc_attr( $platform->slug ); ?>" href="<?php echo esc_url( get_term_link( $platform ) ); ?>">
                                    <?php
                                    // Custom SVGs for card platform
                                    if ( $platform->slug === 'ms-windows' || $platform->slug === 'windows' ) {
                                        echo '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" style="height:14px;"><path d="M0 80v160h224V52zm256-32v192h256V16zm0 224v192l256 32V272zM0 272v160l224 28V272z"></path></svg>';
                                    } elseif ( $platform->slug === 'mac' || $platform->slug === 'macos' ) {
                                        echo '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512.003 512.003" style="height:14px;"><path d="M351.98 0c-27.296 1.888-59.2 19.36-77.792 42.112-16.96 20.64-30.912 51.296-25.472 81.088 29.824.928 60.64-16.96 78.496-40.096 16.704-21.536 29.344-52 24.768-83.104"></path><path d="M459.852 171.776c-26.208-32.864-63.04-51.936-97.824-51.936-45.92 0-65.344 21.984-97.248 21.984-32.896 0-57.888-21.92-97.6-21.92-39.008 0-80.544 23.84-106.88 64.608-37.024 57.408-30.688 165.344 29.312 257.28 21.472 32.896 50.144 69.888 87.648 70.208 33.376.32 42.784-21.408 88-21.632 45.216-.256 53.792 21.92 87.104 21.568 37.536-.288 67.776-41.28 89.248-74.176 15.392-23.584 21.12-35.456 33.056-62.08-86.816-33.056-100.736-156.512-14.816-203.904"></path></svg>';
                                    } elseif ( strpos( $platform->slug, 'android' ) !== false ) {
                                        echo '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512.007 512.007" style="height:14px;"><path d="M64.004 192.007c-17.664 0-32 14.336-32 32v128c0 17.664 14.336 32 32 32s32-14.336 32-32v-128c0-17.664-14.336-32-32-32m384 0c-17.664 0-32 14.336-32 32v128c0 17.664 14.336 32 32 32s32-14.336 32-32v-128c0-17.664-14.336-32-32-32m-320 1.856v192c0 17.664 14.336 32 32 32v62.144c0 17.664 14.336 32 32 32s32-14.336 32-32v-62.144h64v62.144c0 17.664 14.336 32 32 32s32-14.336 32-32v-62.144c17.664 0 32-14.336 32-32v-192zm0-33.856h256c0-40.32-19.008-75.84-48.128-99.296l28.48-34.528c5.632-6.816 4.672-16.896-2.144-22.528-6.848-5.6-16.896-4.672-22.528 2.144l-31.136 37.728c-16.064-7.264-33.76-11.52-52.544-11.52-19.04 0-36.96 4.416-53.184 11.904L172.516 6.023c-5.536-6.88-15.584-8.032-22.496-2.496-6.88 5.536-8 15.584-2.496 22.496l28.096 35.136c-28.832 23.456-47.616 58.784-47.616 98.848m160-80c8.832 0 16 7.168 16 16s-7.168 16-16 16-16-7.168-16-16 7.168-16 16-16m-64 0c8.832 0 16 7.168 16 16s-7.168 16-16 16-16-7.168-16-16 7.168-16 16-16"></path></svg>';
                                    } elseif ( $platform->slug === 'pc-games' ) {
                                    ...
                                    ?>
                                    <span><?php echo esc_html( $platform->slug ); ?></span>
                                </a>
