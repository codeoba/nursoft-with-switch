<?php
/**
 * The template for displaying all single book items
 *
...
<main class="container main_content">
    ...
    while ( have_posts() ) :
        ...
        $format_display      = $format !== '' ? esc_html($format) : 'PDF';

        // Platform & Category Terms
        $platforms = wp_get_post_terms( get_the_ID(), 'platform' );
        $platform_name = ! empty( $platforms ) && ! is_wp_error( $platforms ) ? $platforms[0]->name : 'Books';
        
        $categories = wp_get_post_terms( get_the_ID(), 'book_cat' );
        ?>
        ...
        <div class="software_layout">
            ...
            <article class="software_main_col">
                ...
                <header class="soft_ident_header">
                    <div class="soft_ident_icon_wrap" style="position: relative; flex-shrink: 0;">
                        <div class="soft_ident_icon" style="border-radius: 4px; overflow: hidden; width: 80px; height: 110px;">
                            <?php if ( has_post_thumbnail() ) : ?>
                                <?php the_post_thumbnail( 'thumbnail', array( 'alt' => get_the_title(), 'style' => 'object-fit: cover; width: 100%; height: 100%;' ) ); ?>
                            <?php else : ?>
                                <img src="<?php echo get_template_directory_uri(); ?>/assets/img/default-logo.png" alt="Book cover" style="object-fit: cover; width: 100%; height: 100%;" />
                            <?php endif; ?>
                        </div>
                        <?php 
                    ...
                    <div class="soft_ident_info" style="margin-left: 20px;">
                        ...
                        <div class="soft_ident_cat_list">
                            ...
                            <?php endif; ?>
                            <?php if ( $author ) : ?>
                                <span class="soft_ident_cat" style="background-color: rgba(94, 114, 228, 0.1); color: #5e72e4; border: 1px solid rgba(94, 114, 228, 0.2);">
                                    <?php printf( __('By %s', 'nursoft'), esc_html($author) ); ?>
                                </span>
                            <?php endif; ?>
                        </div>
                    </div>
                    ...
                    <div class="soft_fav_action" style="margin-left:auto; display:flex; align-items:center; justify-content:center; padding:10px;">
                        <button class="fav-toggle-btn" data-post-id="<?php the_ID(); ?>" data-version="<?php echo esc_attr( $format_display ); ?>" title="<?php esc_attr_e( 'Bookmark this', 'nursoft' ); ?>">
                ...
                <div class="soft_tech_grid" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(110px, 1fr)); gap: 10px; margin: 25px 0;">
                    ...
                    <div class="soft_tech_card">
                        <div class="soft_tech_label"><?php _e('Author', 'nursoft'); ?></div>
                        <div class="soft_tech_value" style="font-size: 13px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;" title="<?php echo esc_attr($author); ?>"><?php echo $author ? esc_html($author) : 'N/A'; ?></div>
                    </div>
                    <!-- Format -->
                    <div class="soft_tech_card">
                        <div class="soft_tech_label"><?php _e('Format', 'nursoft'); ?></div>
                        <div class="soft_tech_value"><?php echo $format_display; ?></div>
                    </div>
                    <!-- Pages -->
                    <div class="soft_tech_card">
                        <div class="soft_tech_label"><?php _e('Pages', 'nursoft'); ?></div>
                        <div class="soft_tech_value"><?php echo $pages ? esc_html($pages) : 'N/A'; ?></div>
                    </div>
                    <!-- Size -->
                    <div class="soft_tech_card">
                ...
                <section class="soft_desc_content">
                    <h5 class="soft_section_title">
                        ...
                        <span><?php _e('Book Description', 'nursoft'); ?></span>
                    </h5>
                    <?php the_content(); ?>
                </section>
                ...
                <section class="requirements_box" style="margin-top: 30px;">
                    <h5 class="requirements_title"><?php _e('Book Specifications', 'nursoft'); ?></h5>
                    <div class="requirements_content" style="line-height: 1.8;">
                        <strong><?php _e('Title:', 'nursoft'); ?></strong> <?php the_title(); ?><br/>
                        <?php if($author) : ?><strong><?php _e('Author:', 'nursoft'); ?></strong> <?php echo esc_html($author); ?><br/><?php endif; ?>
                        <?php if($pages) : ?><strong><?php _e('Pages:', 'nursoft'); ?></strong> <?php echo esc_html($pages); ?><br/><?php endif; ?>
                        <?php if($format) : ?><strong><?php _e('Format:', 'nursoft'); ?></strong> <?php echo esc_html($format); ?><br/><?php endif; ?>
                        <?php if($language) : ?><strong><?php _e('Language:', 'nursoft'); ?></strong> <?php echo esc_html($language); ?><br/><?php endif; ?>
                        <?php if($size) : ?><strong><?php _e('File Size:', 'nursoft'); ?></strong> <?php echo esc_html($size); ?><br/><?php endif; ?>
                    </div>
                </section>
                ...
                <section class="download_box">
                    ...
                    <h5 class="download_box_title"><?php _e('Ready to Download Book?', 'nursoft'); ?></h5>
                    <p class="download_box_desc"><?php _e('All ebooks are fully verified and pre-scanned. Click down to start.', 'nursoft'); ?></p>
            ...
            <aside class="software_sidebar_col">
                ...
                <div class="widget_box">
                    <h5 class="widget_title"><?php _e('Related Books', 'nursoft'); ?></h5>
                    <div class="widget_list">
                        ...
                        if ( $rel_query->have_posts() ) :
                            while ( $rel_query->have_posts() ) : $rel_query->the_post();
                                $rel_author = get_post_meta( get_the_ID(), '_nursoft_book_author', true );
                                $rel_size   = get_post_meta( get_the_ID(), '_nursoft_size', true );
                                $rel_format = get_post_meta( get_the_ID(), '_nursoft_book_format', true );
                                ?>
                                <div class="widget_item">
                                    <a href="<?php the_permalink(); ?>" class="widget_thumb" style="border-radius: 4px; width: 40px; height: 55px;">
                                        <?php if ( has_post_thumbnail() ) : ?>
                                    ...
                                    <div class="widget_item_info" style="margin-left: 12px;">
                                        ...
                                        <div class="widget_item_meta">
                                            <span style="color:var(--accent-blue);"><?php echo $rel_author ? esc_html($rel_author) : 'Unknown'; ?></span>
                                            <span><?php echo esc_html($rel_size); ?> (<?php echo esc_html($rel_format ? $rel_format : 'PDF'); ?>)</span>
                                        </div>
                                    </div>
                                </div>
                            <?php
                            ...
                            echo '<p style="font-size:13px; color:var(--text-secondary);">' . __('No related books found.', 'nursoft') . '</p>';
                        endif;
                        ?>
                    </div>
                </div>
                ...
                <div class="widget_box">
                    <h5 class="widget_title"><?php _e('Popular Ebooks', 'nursoft'); ?></h5>
                    <div class="widget_list">
                        <?php
                        $pop_args = array(
                            'post_type'      => 'book',
                        ...
                        if ( $pop_query->have_posts() ) :
                            while ( $pop_query->have_posts() ) : $pop_query->the_post();
                                $pop_author = get_post_meta( get_the_ID(), '_nursoft_book_author', true );
