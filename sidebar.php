...
<aside class="software_sidebar_col main_sidebar">
    ...
    } elseif ( is_singular( 'book' ) || is_tax( 'book_cat' ) || is_post_type_archive( 'book' ) ) {
        $target_taxonomy = 'book_cat';
        $current_plat_name = __( 'Books', 'nursoft' );
    } elseif ( is_singular( 'course' ) || is_tax( 'course_cat' ) || is_post_type_archive( 'course' ) ) {
        $target_taxonomy = 'course_cat';
        $current_plat_name = __( 'Courses', 'nursoft' );
    } elseif ( is_singular( 'software' ) || is_tax( 'software_cat' ) || is_post_type_archive( 'software' ) ) {
        $target_taxonomy = 'software_cat';
        $current_plat_name = __( 'Software', 'nursoft' );
    }

    if ( ! empty( $target_taxonomy ) ) :
        // Load all terms for the active target taxonomy
